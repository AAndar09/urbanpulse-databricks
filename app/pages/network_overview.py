...
def build_layout():
    ...
    Databricks SQL queries are deliberately NOT executed here.
    The one-shot interval below triggers the data-loading
    callback after Dash has rendered the route.
    """
    ...
    return html.Main(
        [
            build_page_header(
                ...
                "Network Overview",
                (
                    "Operational intelligence for the "
                    "London Underground network."
                ),
            ),

            dcc.Interval(
                id="network-overview-loader",
                interval=500,
                n_intervals=0,
                max_intervals=1,
            ),
...
def build_network_overview_content():
    # -----------------------------------------------------
    ...
    return html.Div(
        [
            ...
            build_section_title(
                "Network Health",
                (
                    "Latest operational state across "
                    "monitored Tube lines."
                ),
            ),
...
@callback(
    ...
    Input(
        "network-overview-loader",
        "n_intervals",
    ),
    prevent_initial_call=True,
)
def load_network_overview(_):
    try:
        return build_network_overview_content()
